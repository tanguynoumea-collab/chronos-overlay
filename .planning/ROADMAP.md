...
- [x] **Phase 24 : L'arbitrage par fraîcheur** - Un signal n'en écrase un autre que s'il est plus récent, jamais par ordre d'insertion, et les désaccords deviennent traçables
 (completed 2026-09-12)
- [x] **Phase 25 : Le contrat d'événements refondé** - `PermissionRequest` au lieu du proxy `Notification`, « réfléchit » observé par battements de cœur, interruption utilisateur couverte, contrat documenté (completed 2026-09-12)
- [x] **Phase 26 : « Traité » veut enfin dire quelque chose** - Déduit d'une transition observée sur la même source, persistant au redémarrage, assorti d'un geste explicite et d'un contrat d'archivage unique (completed 2026-09-12)
...
- [x] 25-03-PLAN.md — EVT-04 : l'attente DÉDUITE, visible et dite comme telle, jamais confondue avec un « tour fini »
- [x] 25-04-PLAN.md — EVT-05 : le contrat des hooks écrit dans `docs/`, avec ses trois trous documentaires, et la garde qui l'empêche de mentir
**UI hint**: yes
...
**Plans**: 4 plans, 4 vagues strictement sérielles. Les quatre se disputent `App.xaml.cs`, `TreatedStore.cs`,
`ArchiveStore.cs` et les mêmes fichiers de tests : la propriété exclusive des fichiers impose l'ordre. Cet
ordre est aussi celui de la dépendance logique — le détecteur (26-01) doit dire la vérité avant que le
magasin d'archives puisse tenir sa promesse (26-02), et le geste explicite (26-03) ne peut annoncer
« réversible » et « définitif » que si les deux contrats sont déjà vrais. Le document (26-04) ne décrit
...
- [x] 26-02-PLAN.md — TRT-04 : le magasin d'archives perd sa durée de vie de six heures et gagne une horloge injectable — ce qui est archivé ne revient jamais
- [x] 26-03-PLAN.md — TRT-03 (+ volet UI de TRT-04) : le geste explicite et le geste de masse sur les 8 styles et les 9 thèmes, avec des libellés qui disent s'ils reviennent
- [x] 26-04-PLAN.md — le §3 du contrat des hooks suit le câblage, une garde croisée document/code le tient, et la carte de validation se clot sur des chiffres mesurés
**UI hint**: yes
...
| 26. « Traité » veut enfin dire quelque chose | 4/4 | Complete   | 2026-09-12 |
