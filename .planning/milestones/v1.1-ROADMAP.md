...
**Phases:** 8-9 · **Total Plans:** 5

## Overview

Adaptation au workflow réel de l'utilisateur (100 % app bureau, statusline jamais rendue) : le repli
JSONL devient pleinement utile sans trahir l'honnêteté. Reset 5 h inféré de l'activité (trou ≥ 5 h),
utilization estimée via plafonds calibrables (manuel + auto opportuniste), tokens estimés surfacés,
dette v1.0 nettoyée (event mort, champ Age), fix staleness (capturedAt suit la source affichée).

**Key accomplishments :**
1. FiveHourWindowInference : la fenêtre 5 h se déduit des transcripts réels — l'arc retrouve une longueur en mode bureau.
2. Plafonds calibrables (menu « Calibrer les plafonds… ») avec règle de priorité Manual > Auto, persistés, appliqués sans redémarrage.
3. Auto-calibration opportuniste inerte par défaut (tokens/used_percentage mémorisé si un rate_limits réel passe).
4. Tokens estimés affichés (« ≈ 128,4 M tokens ») — la matière première de l'estimation est visible.
5. Contrat providers nettoyé (NET-01) + fix honnêteté staleness (composite).

**Stats :** 152 tests verts · vérifié E2E sur les données réelles du poste.
