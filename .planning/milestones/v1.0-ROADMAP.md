# Milestone v1.0 : Chronos — Overlay de quotas Claude
...
Première version complète de Chronos : overlay WPF always-on-top en forme d'horloge affichant
l'état des limites d'usage Claude (5 h + hebdo) — deux arcs concentriques (longueur = temps restant,
couleur = utilization, gris = épuisé), countdown central, honnêteté des chiffres (badges « estimée »
par fenêtre, jamais de valeur inventée), drag + snap coins multi-écrans, menu contextuel, persistance,
recalibrage hebdo, autostart, exe self-contained mono-fichier 74 Mo.

**Stats :** 123 commits · 92 fichiers source · ~5 200 LOC · 107 tests verts · audit intégration 6/6 flux E2E.

**Key accomplishments :**
1. Découverte de la source primaire : bloc rate_limits du contrat statusLine officiel + pont Node non destructif statusLine→usage.json installé avec backup.
2. Pipeline de données neutre (IUsageProvider, snapshots immuables, composite par fenêtre) avec modèle d'honnêteté nullable — le repli JSONL n'invente jamais d'utilization.
3. Deux horloges disjointes : watcher débouncé + PeriodicTimer (données) et DispatcherTimer 1 s d'interpolation pure (UI), marshaling unique IUiDispatcher.
4. Cadran XAML pur fidèle à la maquette (RingArc Shape, TickRing, rampe vert→ambre→rouge→gris, tokens exacts).
5. Comportements overlay complets : placement en pixels physiques (contourne le bug WPF PerMonitorV2), snap coins, menu clic droit, persistance atomique, recalibrage gardé par Exact.
6. Packaging mono-fichier prouvé (publish réel + smoke) ; GAP-1 d'intégration détecté et corrigé pendant l'audit.
...
- [x] **Phase 1 : Fondations architecture + squelette overlay** - Fenêtre overlay vide, borderless/transparente/always-on-top portée par un graphe DI
- [x] **Phase 2 : Découverte des sources (bloquante)** - Localisation et documentation empirique de l'objet d'usage Claude AVANT tout code de provider
- [x] **Phase 3 : Modèles + pipeline de données** - Snapshots neutres et providers primaire/repli/composite isolés du cadran
- [x] **Phase 4 : Orchestration refresh + ViewModel temps réel** - Rafraîchissement automatique et interpolation à la seconde sans erreur de thread
- [x] **Phase 5 : Cadran (RingArc + converters) + câblage View** - Cadran à deux anneaux reflétant temps restant et utilization en temps réel
- [x] **Phase 6 : Comportements overlay (placement + interaction)** - Glisser/accroche, menu contextuel, persistance, autostart et recalibrage
- [x] **Phase 7 : Packaging + déploiement** - Exe self-contained mono-fichier win-x64 testé sur machine propre
