...
**Phases:** 10-11 · **Total Plans:** 4

## Overview

Chronos affiche désormais les chiffres EXACTS des deux fenêtres, automatiquement, sans terminal ni
calibration : un provider rejoue `GET https://api.anthropic.com/api/oauth/usage` (l'endpoint officiel
de l'app bureau) avec le token OAuth local, déchiffré du coffre safeStorage `v10` (DPAPI + AES-256-GCM).
Devient la source primaire du composite (OAuth → pont statusLine → repli JSONL) ; le badge « estimée »
disparaît sur les fenêtres exactes. Réglage menu on/off ; désactivé = zéro accès au token.

**Sécurité :** token jamais logué/écrit/exposé, uniquement en en-tête Authorization vers api.anthropic.com,
lecture seule du coffre, tolérance totale (toute erreur → repli), audit sécurité passé.

**Key accomplishments :**
1. Test décisif E2E réel réussi (déchiffrement + appel → vrais % : 65/92 puis 74/93 au fil de l'usage).
2. ClaudeTokenReader : déchiffrement safeStorage v10 tolérant, non-écriture prouvée par test.
3. ClaudeOAuthUsageProvider : mapping utilization/ISO → Exact, inertie hors ligne / token expiré, tolérance 401/timeout/malformé.
4. Chaîne composite à 3 niveaux par imbrication (best-par-fiabilité généralisé), toggle « Usage exact (OAuth) » persisté (GAP-1).

**Stats :** 188 tests verts · vérifié en app réelle (arcs colorés exacts, sans badge).
