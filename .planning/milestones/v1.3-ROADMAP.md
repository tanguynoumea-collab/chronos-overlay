...
**Phases:** 12 · **Total Plans:** 2 (+ 4 raffinements directs testés)

## Overview

Refonte visuelle du cadran demandée par l'utilisateur, plus des raffinements en cours d'itération.
Aucun changement des sources de données ; pur rendu/ViewModel + une correction de robustesse OAuth.

**Key accomplishments :**
1. Remplissage INVERSÉ des arcs (se remplissent vers le reset) + correctif « reset inconnu = arc vide ».
2. 3 anneaux réordonnés et distincts : hebdo (interne) → 5 h (milieu) → timeline 24 h (externe, fin, rapproché).
3. Anneau 24 h : rempli selon l'heure du jour, gradué aux resets 5 h projetés, couleur = utilization 5 h.
4. Centre épuré : uniquement les deux pourcentages ; marques horaires (5 h) + resets (24 h) bien visibles.
5. Compacité 170 px + fond transparent (anneaux flottant sur le bureau, petit disque central pour les %).
6. Clic au centre : bascule pourcentages ↔ temps avant reset (sans déclencher le drag).
7. **Résilience anti-429 du provider OAuth** (throttle 2 min, backoff 5 min, conservation de l'exact) —
   corrige le clignotement exact↔estimé (« infos étranges ponctuelles »).

**Stats :** 215 tests verts · vérifié en app réelle (2 modes capturés, fond transparent, données exactes).
